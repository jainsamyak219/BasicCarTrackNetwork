...
function Org1CCP {
    ...
    echo "$(json_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > ./api-server/connection-org1.json
   # echo "$(yaml_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > ./api-server/connection-org1.yaml
...
function Org2CCP {
    ...
    echo "$(json_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > ./api-server/connection-org2.json
   # echo "$(yaml_ccp $ORG $P0PORT $CAPORT $PEERPEM $CAPEM)" > ./api-server/connection-org2.yaml
